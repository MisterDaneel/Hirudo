...
class TORRENT9API:
    ...
    def search(self, query):
        ...
        soup = bs(response, "lxml")
        table = soup.find('table',attrs={"class":"table table-striped table-bordered cust-table"})
        if table == None:
            #raise ValueError('Aucun torrents disponibles correspondant a votre recherche')
            return 0
        ...
        for torrent in tbody.findAll('tr'):
            ...
            response.append( {'name': name, 'href': href, 'size': size, 'seeders': seeders, } )
            #except:
            #    None
        if len(response) == 0:
            return 0
